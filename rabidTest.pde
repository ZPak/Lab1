...
void draw(){
      ...
      if(grid[x][y].isRabid()){
        if(grid[x][y].getNeighbors().size() > 0){
          for(int i=0; i<grid[x][y].getNeighbors().size(); i++){
            if(!(grid[x][y].getNeighbors().get(i).isRabid())){
  ...
  numGenerations++;
  if(numGenerations == UNLEASH_THE_HOUNDS){
    for(int i=0; i<40; i++){
...
class Scorpion{
  ...
  void setRabid(boolean r){
    rabid = r;
  }
  
  void makeRabid(){
    rabid = true;
    vitality = MAX_HEALTH*5;
    tempVitality = MAX_HEALTH*5;
  }
  ...
  void walk(){
    ...
    if(direction == 0){
      if(y-1 >= 0){
        if(rabid){
          grid[x][y-1].setTempVitality(this.getVitality());
          grid[x][y-1].setRabid(true);
          grid[x][y].death();
          split(x,y);
          grid[x][y-1].setXY(x, y-1);
        } else if (grid[x][y-1].getVitality() <= 0){
      ...
      if((y-1 >= 0) && (x+1 < cols)){
        if(rabid){
          grid[x+1][y-1].setTempVitality(this.getVitality());
          grid[x+1][y-1].setRabid(true);
          grid[x][y].death();
          split(x,y);
          grid[x+1][y-1].setXY(x+1, y-1);
        } else if(grid[x+1][y-1].getVitality() <= 0){
      ...
      if(x+1 < cols){
        if(rabid){
          grid[x+1][y].setTempVitality(this.getVitality());
          grid[x+1][y].setRabid(true);
          grid[x][y].death();
          split(x,y);
          grid[x+1][y].setXY(x+1, y);
        } else if(grid[x+1][y].getVitality() <= 0){
